...
PROCEDURE PA_SAI_CATEGORIES_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, ods VARCHAR2:=NULL,tipo VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_ODS_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,tipo VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_TYPE_REPORT_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,ods VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_COUNTRY_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,tipo VARCHAR2:=NULL, ods VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_AUDIT_LIST(usuario VARCHAR2:=NULL, estado VARCHAR2:=NULL,c_auditorias OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_LIST_OF_REASONS(ids VARCHAR2:=NULL, c_motivos OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_LIST_USERS(ids integer:=NULL, c_usuarios2 OUT SYS_REFCURSOR);

PROCEDURE PA_SAI_UPDATE_STATUS_AUDITORIA(ids IN OUT INTEGER, status IN INTEGER:=NULL, usuario in varchar2:=NULL);
...
BEGIN
        INSERT INTO SAI_AUDITORIA(   ID              ,     REPORT_TITLE     ,   RERPOR_OBJETIVE  ,
                                     ...
                                     REPORT_IMAGE     ,
                                     CREATED_AT       , ID_USER,id_status)
                        VALUES  (ids      , titulo     , objetivo   , resumen    , categoria        ,
                                 tipo     , fini       , ffin       ,
                                 imagen     , SYSDATE, usuario,1)
...
PROCEDURE PA_SAI_UPDATE_USUARIO(
    ...
    BEGIN
        ...
        and id_status = 4; -- solo aprobados
    END;
    ...
    BEGIN     
        ...
        */
        ...
        sql_stmt:= sql_stmt || ' and a.id_status = 4 ';
    ...
    BEGIN     
        sql_stmt := '
        select b.ambito_id AMBITO_ID, C.GROUP_NAME AMBITO, a.type_report_id TIPO_ID, d.type_report_name TIPO,count(distinct a.id) CANTIDAD from sai_auditoria a 
inner join sai_participante b
on a.id = b.report_id and a.id_status = 4
inner join sai_ambito c
on b.ambito_id = c.id
inner join sai_tipo_auditoria d
on d.id = a.type_report_id
inner join sai_auditoria_ods e
on a.id = e.report_id
        ';
...
count(distinct a.id) CANTIDAD from sai_auditoria a 
inner join sai_participante b
on a.id = b.report_id  and a.id_status = 4
inner join sai_ambito c
on b.ambito_id = c.id
...
on a.id = e.report_id
    ...
    PROCEDURE PA_SAI_CATEGORIES_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, ods VARCHAR2:=NULL,tipo VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR) IS 
    sql_stmt varchar2(4000);
    sql_where varchar2(1000);
    BEGIN     
        sql_stmt := '
select 
a.category_id TEMA_ID,
f.category_name TEMA,
count(distinct a.id) CANTIDAD 
from sai_auditoria a 
inner join sai_participante b
on a.id = b.report_id and a.id_status = 4
inner join sai_ambito c
...
on a.category_id = f.id
        ...
        sql_stmt := sql_stmt || ' group by a.category_id,f.category_name order by 1 asc ';
    ...
    PROCEDURE PA_SAI_ODS_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,tipo VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR) IS 
    sql_stmt varchar2(4000);
    sql_where varchar2(1000);
    BEGIN     
        sql_stmt := '
...
e.ods_id ODS_ID,
g.ods_name ODS,
a.type_report_id TIPO_ID,
d.type_report_name TIPO,
count(distinct a.id) CANTIDAD 
from sai_auditoria a 
inner join sai_participante b
on a.id = b.report_id and a.id_status = 4
...
on g.id = e.ods_id
        ...
        sql_stmt := sql_stmt || ' group by e.ods_id,g.ods_name,a.type_report_id,d.type_report_name ';
    ...
    PROCEDURE PA_SAI_TYPE_REPORT_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,ods VARCHAR2:=NULL, pais VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR) IS 
    sql_stmt varchar2(4000);
    sql_where varchar2(1000);
    BEGIN     
        sql_stmt := '
...
on g.id = e.ods_id
        ...
        sql_stmt := sql_stmt || ' group by a.type_report_id,d.type_report_name ';
    ...
    PROCEDURE PA_SAI_COUNTRY_GRAPH(ambito VARCHAR2:=NULL, mes VARCHAR2:=NULL, categoria VARCHAR2:=NULL,tipo VARCHAR2:=NULL, ods VARCHAR2:=NULL,anio varchar2:=NULL,c_auditorias OUT SYS_REFCURSOR) IS 
    sql_stmt varchar2(4000);
    sql_where varchar2(1000);
    BEGIN     
        sql_stmt := '
...
ON b.pais_id=h.id
        ...
        sql_stmt := sql_stmt || ' group by h.code,b.pais_id ';
...
Select rol_id into V_ROL from sai_usuarios where ID = usuario;
    OPEN c_auditorias for
    select 
        A.ID REPORT_ID, REPORT_TITLE, (case when a.id_status=1 then A.CREATED_AT else A.UPDATED_AT end)
        REPORT_DATE,
        a.id_status STATUS_ID,P.value_name STATUS
    from sai_auditoria A
    INNER JOIN SAI_PARAMETRO P
    ON a.id_status=p.CODE AND p.table_name='TABLE_STATUS'
    WHERE 
    a.id_status = estado
    AND (ID_USER = usuario or 1=V_ROL);
    ...
    SELECT 
        ID MOTIVO_ID,
        MOTIVO,
        CREATED_AT MOTIVO_FECHA
    FROM SAI_OBSERVACION 
    WHERE REPORT_ID = ids;
    ...
    OPEN c_usuarios2 for
    select 
        FIRST_NAME NOMBRES, LAST_NAME APELLIDOS, EMAIL,
        U.PAIS_ID GRUPO_SAI_ID, P.COUNTRY_NAME GRUPO_SAI, CASE WHEN GENERO='M' THEN 'MASCULINO' WHEN GENERO='F' THEN 'FEMENINO' ELSE 'INDEFINIDO' END SEXO, 
        ROL_ID, R.NAME ROL,CASE WHEN IS_ACTIVE=1 THEN 'ACTIVO' ELSE 'INACTIVO' END ACTIVO
    from sai_usuarios U
    INNER JOIN SAI_PAIS P
    ON U.PAIS_ID=P.ID
    INNER JOIN SAI_ROLES R
    ON U.ROL_ID =R.ID
    WHERE (ids is null or u.ID=ids);
