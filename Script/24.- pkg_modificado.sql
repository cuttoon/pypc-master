...
PROCEDURE PA_SAI_GET_AUDITORIA_ADVANCE(idiomas VARCHAR2:=NULL, ambito VARCHAR2:=NULL, pais VARCHAR2:=NULL,inicio VARCHAR2:=NULL, fin VARCHAR2:=NULL,tipo varchar2:=NULL, categoria varchar2:=NULL, ods VARCHAR2:=NULL,c_auditorias OUT SYS_REFCURSOR);
...
PROCEDURE PA_SAI_UPDATE_USUARIO(
    ...
    PROCEDURE PA_SAI_GET_AUDITORIA_ADVANCE(idiomas VARCHAR2:=NULL, ambito VARCHAR2:=NULL, pais VARCHAR2:=NULL,inicio VARCHAR2:=NULL, fin VARCHAR2:=NULL,tipo varchar2:=NULL, categoria varchar2:=NULL, ods VARCHAR2:=NULL,c_auditorias OUT SYS_REFCURSOR) IS 
    ...
    BEGIN     
        ...
        if (inicio is not null) then
            if LENGTH(sql_where)>0 THEN
                sql_where := sql_where|| ' AND a.created_at >= TO_DATE(''' || inicio || ''', ''YYYYMMDD'') ';
            else
                sql_where := sql_where|| ' a.created_at >= TO_DATE(''' || inicio || ''', ''YYYYMMDD'') ';
            end if;
        end if;
        
        if (fin is not null) then
            if LENGTH(sql_where)>0 THEN
                sql_where := sql_where||' AND a.created_at <= TO_DATE(''' || fin || ''', ''YYYYMMDD'') ';
            else
                sql_where := sql_where||' a.created_at <= TO_DATE(''' || fin || ''', ''YYYYMMDD'') ';
            end if;
        end if;
