    ...
    BEGIN
        OPEN c_users FOR
         SELECT 
                ...
                R.SLUG ROL
                FROM SAI_USUARIOS U
                INNER JOIN SAI_ROLES R ON R.ID = U.ROL_ID                
                ORDER BY U.ID;   
...
PROCEDURE PA_SAI_INSERT_USUARIO(
    ...
    BEGIN     
        ...
        (
            ID,EMAIL,FIRST_NAME,LAST_NAME,PASSWORD,NUM_LOGIN,CREATED_AT, is_active,ROL_ID
        ) 
        values (
            ids,email,nombre,apellido,password,0,SYSTIMESTAMP,1,rol
        )
